...
BUNDLED_DATA_FILE = Path(__file__).with_name("tabigame_japan_realmap_data.json")
...
def init_state() -> None:
    ...
    if not st.session_state.spots:
        ...
        if BUNDLED_DATA_FILE.exists():
            try:
                bundled = json.loads(BUNDLED_DATA_FILE.read_text(encoding="utf-8"))
                if isinstance(bundled, dict) and isinstance(bundled.get("spots"), list):
                    st.session_state.spots = bundled.get("spots", [])
                    st.session_state.trip_start = date.fromisoformat(
                        bundled.get("trip_start", str(st.session_state.trip_start))
                    )
                    st.session_state.trip_days = int(
                        bundled.get("trip_days", st.session_state.trip_days)
                    )
                    st.session_state.day_modes = bundled.get("day_modes", {})
                    st.session_state.map_region = bundled.get(
                        "map_region", st.session_state.map_region
                    )
                    st.session_state.day_start_times = bundled.get(
                        "day_start_times", {}
                    )
                    st.session_state.registered_places = bundled.get(
                        "registered_places", []
                    )
                    st.session_state.budget_settings = bundled.get(
                        "budget_settings",
...
def budget_editor_frame() -> pd.DataFrame:
    """予定20件を、画面で金額・区分・根拠を編集できる表へ変換する。"""
    rows = []
    for spot in sorted(
        st.session_state.spots,
        key=lambda item: (
            str(item.get("day", "")),
            int(item.get("manual_order", 9999) or 9999),
            str(item.get("arrival", item.get("start", ""))),
        ),
    ):
        rows.append({
            "UID": str(spot.get("_uid", "")),
            "日付": str(spot.get("day", "")),
            "場所名": str(spot.get("name", "")),
            "カテゴリ": str(spot.get("category", "")),
            "予算円": int(spot.get("cost", 0) or 0),
            "予算区分": str(spot.get("cost_group", "要確認")),
            "予算根拠": str(
                spot.get("cost_basis", spot.get("cost_note", ""))
            ),
        })
    return pd.DataFrame(rows)


def apply_budget_editor(edited: pd.DataFrame) -> int:
    """予算編集表の変更を、UIDで元予定へ戻す。"""
    spot_map = {
        str(spot.get("_uid", "")): spot
    ...
    for _, row in edited.iterrows():
        uid = str(row.get("UID", ""))
        spot = spot_map.get(uid)
        if spot is None:
            continue

        new_cost = int(row.get("予算円", 0) or 0)
        new_group = str(row.get("予算区分", "") or "要確認")
        new_basis = str(row.get("予算根拠", "") or "")

        if (
            int(spot.get("cost", 0) or 0) != new_cost
            or str(spot.get("cost_group", "要確認")) != new_group
            or str(spot.get("cost_basis", spot.get("cost_note", ""))) != new_basis
        ):
            changed += 1

        spot["cost"] = new_cost
        spot["cost_group"] = new_group
        spot["cost_basis"] = new_basis
        spot["cost_people"] = 2
...
if st.session_state.page == "旅程":
    ...
    with center:
        if tool == "GoogleマップURL登録":
            ...
            st.markdown("#### 全予定の場所別予算")
            st.caption(
                f"現在の予定は{len(st.session_state.spots)}件です。"
                "予算円・予算区分・予算根拠は、この表から直接変更できます。"
            )

            budget_df = budget_editor_frame()
            edited_budget_df = st.data_editor(
                budget_df,
                use_container_width=True,
                hide_index=True,
                disabled=["UID", "日付", "場所名", "カテゴリ"],
                column_config={
                    "UID": None,
                    "日付": st.column_config.TextColumn("日付", width="small"),
                    "場所名": st.column_config.TextColumn("場所名", width="large"),
                    "カテゴリ": st.column_config.TextColumn("カテゴリ", width="medium"),
                    "予算円": st.column_config.NumberColumn(
                        "2人分の予算",
                        min_value=0,
                        step=100,
                        format="%d円",
                    ...
                    "予算区分": st.column_config.SelectboxColumn(
                        "予算区分",
                        options=[
                            "飲食",
                            "観光・体験",
                    ...
                    "予算根拠": st.column_config.TextColumn(
                        "料金の根拠",
                        width="large",
                    ),
                },
                key="all_spot_budget_editor",
            )
            ...
            if st.button(
                ...
                type="primary",
                use_container_width=True,
                key="apply_budget_editor_changes",
            ):
                changed = apply_budget_editor(edited_budget_df)
                st.success(f"{changed}件の予算変更を反映しました。")
                st.rerun()
            ...
            for day in trip_dates():
                ...
                day_spots = [
                    spot for spot in st.session_state.spots
                    if str(spot.get("day", "")) == day_text
                ]
                day_total = sum(int(spot.get("cost", 0) or 0) for spot in day_spots)
                st.write(f"**{day.strftime('%m月%d日')}：{day_total:,}円**")
                for spot in day_spots:
                    st.caption(
                        f"{spot.get('name', '')}："
                        f"{int(spot.get('cost', 0) or 0):,}円"
                        f"｜{spot.get('cost_basis', spot.get('cost_note', '未確認'))}"
                    )
